...
export default class WebBuilder {
    ...
    #path_Build;
    #parsers;
    constructor(name, path, buildPath) {
        this.#name = name;
        this.#path = path;
    ...
    async buildLib_Async(parseTS) {
        ...
        for (let i = 0; i < filePaths.length; i++) {
            buildFilePromises.push(this.#build_File_Async(filePaths[i], builtFilePaths, parseTS));
        }
        await Promise.all(buildFilePromises);
        return builtFilePaths;
    }
    async buildPkg_Async(parseTS) {
        this.#createBuildPath();
        let filePaths = await abFSMatcher.getPaths_Async([this.#path +
                (parseTS ? '/**/*.js' : '/**/*.ts')]);
        let buildFilePromises = [];
    ...
    async buildScript_Async(filePath, parseTS) {
        this.#createBuildPath();
        let builtFilePaths = [];
        await this.#build_File_Async(filePath, builtFilePaths, parseTS);
        return builtFilePaths[0];
    }
    async #build_File_Async(filePath, builtFilePaths, parseTS) {
        let data = fs.readFileSync(filePath, "utf-8");
        let basename = path.basename(filePath);
        let relativePath = path.relative(path.resolve(this.#path), filePath);
        ...
        let modulePath = this.#parseModulePath(relativePath === 'index.js' ?
            'index.js' :
            (basename === 'index.js' ?
